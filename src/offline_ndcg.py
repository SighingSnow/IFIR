...
for model in models:
    for d in datasets:
        ...
        if d == 'nfcorpus':
            # select data according to all '_id' in test_data
            test_ids = [c['_id'] for c in test_data]
            data = [d for d in data if d['_id'] in test_ids]
        ...
        # assert results['no_inst']['nDCG'] == sum(local_record['origin'])/ len(local_record['origin'])
        # only 6 digits after the decimal point
        # assert round(results['no_inst']['nDCG'], 6) == round(sum(local_record['origin'])/ len(local_record['origin']), 6)
        
        results['inst']['nDCG'] = mrr_inst
        for i in range(1, 4):
            if results.get('level') is None:
                continue
            if results.get('level').get(str(i)) is None:
                continue
            results['level'][str(i)]['nDCG'] = sum(local_record[str(i)])/ len(local_record[str(i)])
